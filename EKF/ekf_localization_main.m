...
for n = t
  ...
  wt = ut(2);                           % ロボットの角速度
  theta = xt(3);
  vtt = 1.05 * vt;                         % ロボットの位置（係数を変更して挙動を確認）
  wtt = 0.95 * wt;                      % ロボットの方位（係数を変更して挙動を確認）
