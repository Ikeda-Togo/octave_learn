...
foot = [0 0 0; 0.6 0.1 0.06; 0.9 0.2 -0.03; 1.2 0.3 0.12; 1.5 0.4 0.03; 1.8 0.5 0.18; 2.1 0.6 0.09; 2.4 0.6 0.15; 100 0 0];
...
Q = zeros(4);
Q(1) = 10^4;                            % ★★★最適制御の重み係数　大：ZMPが理想に近づく
H = 1;                                  % 同じく重み係数　大：入力が少なくなる．

P = dare(Phai, G, Q, H);                % 離散時間系でのリカッチ方程式の解
F = -inv(H+G'*P*G)*G'*P*Phai;                                % ★★★フィードバックゲイン 式(2.13)
...
% fd = (H+G'*P*G)^(-1)*G'*(xi')^1*P*Gd;	% 外乱除去用　未使用
  ...
  for ttt = tt : dt : (tt + forward_period)
    ...
    if (prefx(i+j) - prefx(i+j-1)) != 0
      f  = -inv(H+G'*P*G)*G'*(xi'^(j-1))*P*GR;% ★★★式(2.14)
